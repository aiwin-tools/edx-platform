...
USER_SENT_REMINDER_EMAIL_SAVE_FOR_LATER = 'edx.bi.user.saveforlater.reminder.email.sent'
...
def _get_event_properties(data):
    ...
    event_properties = {
        ...
        'name': 'user.send.save.for.later.reminder.email' if data.get('reminder') else 'user.send.save.for.later.email',
    }
...
def send_email(email, data):
    ...
    try:
        ...
        tracker.emit(
            USER_SENT_REMINDER_EMAIL_SAVE_FOR_LATER if data.get('reminder') else USER_SENT_EMAIL_SAVE_FOR_LATER,
            event_data
        )
