...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        saved_courses_ids = SavedCourse.objects.\
        ...
        for batch_num in range(int(num_batches)):
            ...
            query = SavedCourse.objects.\
                filter(id__in=saved_courses_batch_ids).order_by('-modified')
            ...
            for saved_course in saved_courses:
                ...
                if not email_sent:
                    logging.info("Unable to send reminder email to {user} for {course} course"
                                 .format(user=str(saved_course.user_id), course=str(saved_course.course_id)))
            saved_courses.update(reminder_email_sent=True)
