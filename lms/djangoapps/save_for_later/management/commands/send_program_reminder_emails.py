...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        saved_program_ids = SavedProgram.objects.\
            filter(reminder_email_sent=False, modified__lt=reminder_email_threshold_date).values_list('id', flat=True)
        ...
        for batch_num in range(int(num_batches)):
            ...
            query = SavedProgram.objects. \
                filter(id__in=saved_program_batch_ids).order_by('-modified')
            saved_programs = use_read_replica_if_available(query)
            for saved_program in saved_programs:
                user = None
                if saved_program.user_id:
                ...
                if program:
                    ...
                    if not email_sent:
                        logging.info("Unable to send reminder email to {user} for {program} program"
                                     .format(user=str(saved_program.user_id), program=str(saved_program.course_id)))
            saved_programs.update(reminder_email_sent=True)
